...
public class JwtUtil {
    ...
    public Boolean validateToken(String token, UserDetails userDetails) throws Exception {
        AuthUserDetail user = (AuthUserDetail) userDetails;
    ...
    private String generateToken(Map<String, Object> claims) {
        ...
        return Jwts.builder().setClaims(claims).setExpiration(expirationDate).signWith(SignatureAlgorithm.HS512, authProperty.getSecretkey()).compact();
    }
    ...
    private Claims getClaimsFromToken(String token) throws Exception {
        Claims claims = null;
        try {
            claims = Jwts.parser().setSigningKey(authProperty.getSecretkey()).parseClaimsJws(token).getBody();
        } catch (Exception e) {
            new Throwable(e);
        }
        return claims;
